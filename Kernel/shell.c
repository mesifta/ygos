...
void command_handle()
{
	...
	if(str_cmp(command, cmd_ECHO)){     //prints string to screen eg. ECHO hello world
		print_s(command + index + 1);
		print_l();
	}else if(str_cmp(command, cmd_COLOR)){  //changes color eg. COLOR A4 A = bg 4 = fg
		char bg = hex_char_to_num(*(command + index + 1));
		char fg = hex_char_to_num(*(command + index + 2));
		set_screen_color(bg, fg);
	}else if(str_cmp(command, cmd_CLEAR)){  //clears screen eg. CLEAR
		clear_screen();
	}else if(str_cmp(command, cmd_SHUTDOWN)){
		clear_screen();
		mem_set(0xB8000, 0, 160);
